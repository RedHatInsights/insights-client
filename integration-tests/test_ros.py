...
def test_ros_install():
    ...
    logging.debug(f"Command return code: {install.returncode}")
    logging.debug(f"Command STDOUT: {install.stdout.strip()}")
    logging.debug(f"Command STDERR: {install.stderr.strip()}")
    assert install.returncode == 0, f"{PACKAGE} was not installed"

    cat_result = subprocess.run(
        ["cat", CONFIG_FILE], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def test_cleanup_pmlogger_and_ros():
    ...
    stop_result = subprocess.run(
        ["systemctl", "stop", SERVICE], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    assert (
    ...
    uninstall_result = subprocess.run(
        ["dnf", "remove", "-y", PACKAGE], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    assert (
