...
def test_ros_install():
    ...
    :expectedresults:
        ...
        2. ros_collect is set to true in insights-client.conf
    """
...
def test_cleanup_pmlogger_and_ros():
    ...
    stop_result = subprocess.run(
        ["systemctl", "stop", "pmlogger"], capture_output=True, text=True
    )
    assert stop_result.returncode == 0, f"Failed to stop pmlogger: {stop_result.stderr}"

    # Verify pmlogger is inactive
    status_result = subprocess.run(
        ["systemctl", "is-active", "pmlogger"], capture_output=True, text=True
    )
    assert (
        status_result.stdout.strip() != "active"
    ), "pmlogger service is still active after stopping"
    ...
    uninstall_result = subprocess.run(
        ["dnf", "remove", "-y", "insights-client-ros"], capture_output=True, text=True
    )
    assert (
        uninstall_result.returncode == 0
    ), f"Failed to uninstall insights-client-ros: {uninstall_result.stderr}"
