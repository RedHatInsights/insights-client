...
def _get_current_context():
    ...
    try:
        ...
        pytest.skip(
            reason="Cannot read /proc/self/attr/current - SELinux not available"
        )
    except IOError as e:
        pytest.skip(reason=f"Failed to get SELinux context: {e}")
...
def _parse_execve_event_block(event_lines):
    ...
    # 1. Extract Context from SYSCALL
    context_match = re.search(r"(?:subj|scontext)=([^\s]+)", full_text)
    if not context_match:
        return None
    running_context = context_match.group(1)
    ...
    # Return format: (comm_name, source_context/unused, running_context)
    return (comm, running_context, running_context)
...
def _selinux_mode(mode):
    ...
    if original_mode == "Disabled":
        pytest.skip(reason="SELinux is disabled on this system")
...
def test_register_unconfined_t_no_context_change(insights_client):
    ...
    with _selinux_mode("Enforcing"):
        ...
        with SELinuxDenialsChecker() as checker:
            result = insights_client.run("--register", selinux_context=context)
            assert result.returncode == 0, f"Registration failed: {result.returncode}"

            # Verify registration
            def check_registered():
                status = insights_client.run(
                    "--status", check=False, selinux_context=context
                )
        ...
        if denials:
            for scontext in re.findall(r"scontext=([^\s]+)", denials):
                if any(x in scontext for x in ["insights_client_t", "insights_core_t"]):
                    pytest.fail(
                        f"Process transitioned to confined context {scontext}. "
                        f"Denials:\n{denials}"
                    )
