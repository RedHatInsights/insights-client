...
def test_pcp_zeroconf_install():
    install = subprocess.run(
        ["dnf", "install", "-y", PACKAGE], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
    )
    assert install.returncode == 0, f"{PACKAGE} was not installed: {install.stderr}"

    pmlogger_status = subprocess.run(
        [SERVICE], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
    )
    ...
    grep_result = subprocess.run(
        ["pcp", "|", "grep", SERVICE], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, shell=True
    )
    assert grep_result.returncode == 0, f"{SERVICE} is not running"

    cat_result = subprocess.run(["cat", CONFIG_FILE], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
...
def test_pmlogger_running_and_metrics_exist():
    status_result = subprocess.run(
        ["systemctl", "is-active", SERVICE], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
    )
...
def test_cleanup_pmlogger_and_pcp():
    ...
    stop_result = subprocess.run(
        ["systemctl", "stop", SERVICE], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
    )
    assert (
    ...
    uninstall_result = subprocess.run(
        ["dnf", "remove", "-y", PACKAGE], stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
    )
    assert (
