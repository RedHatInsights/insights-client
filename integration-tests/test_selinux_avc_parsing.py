...
Covers both aureport table shapes: the leading-blank-line form used on
this branch (RHEL 9, same as RHEL <= 10.1) and the RHEL 10.2+ form with
no leading blank line (see CCT-2793).
"""
...
# Native aureport table on this branch (RHEL 9): leading blank line before
# "AVC Report". Same shape as RHEL <= 10.1.
AUREPORT_LEADING_BLANK = f"""
AVC Report
===============================================================
...
# RHEL 10.2+ dropped the leading blank line. The parser is format-agnostic,
# so both shapes are covered even though this branch's hosts use the first.
AUREPORT_NO_LEADING_BLANK = f"""AVC Report
===============================================================
# date time comm subj syscall class permission obj result event
===============================================================
{_AVC_ROW}
...
def test_get_avcs_parses_leading_blank_header_format():
    with patch("selinux.subprocess.run", return_value=_mock_run(AUREPORT_LEADING_BLANK)):
...
def test_get_avcs_parses_header_without_leading_blank_line():
    with patch("selinux.subprocess.run", return_value=_mock_run(AUREPORT_NO_LEADING_BLANK)):
        with SELinuxAVCChecker() as checker:
            entries = list(checker.get_avcs(skiplisted=False))
    assert len(entries) == 1
    assert entries[0]["comm"] == "insights-client"
