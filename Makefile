...
install-files:
	...
	install -m 644 data/etc/cert-api.access.redhat.com.pem "$(BUILDROOT)$(SYSCONFDIR)/insights-client/cert-api.access.redhat.com.pem" # TBD remove upon CCT-1812 merged
	...
	install -m 644 data/etc/redhattools.pub.gpg "$(BUILDROOT)$(SYSCONFDIR)/insights-client/redhattools.pub.gpg"
	...
	mkdir -p "$(BUILDROOT)$(LOCALSTATEDIR)/log/insights-client/"
	mkdir -p "$(BUILDROOT)$(LOCALSTATEDIR)/lib/insights/"
	mkdir -p "$(BUILDROOT)$(LOCALSTATEDIR)/cache/insights/"
	mkdir -p "$(BUILDROOT)$(LOCALSTATEDIR)/cache/insights-client/"
