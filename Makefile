BUILDROOT ?=
BINDIR ?= /usr/bin
PYTHON3_SITELIB ?= /usr/lib/python3.9/site-packages
SYSCONFDIR ?= /etc
...
install-files:
	...
	install -m 755 src/redhat-access-insights "$(BUILDROOT)$(BINDIR)/"

	# ./src/insights_client/ -> /usr/lib/python3.9/site-packages/insights_client/
	...
	install -m 644 data/etc/.fallback.json "$(BUILDROOT)$(SYSCONFDIR)/insights-client/.fallback.json"
	install -m 644 data/etc/.fallback.json.asc "$(BUILDROOT)$(SYSCONFDIR)/insights-client/.fallback.json.asc"
